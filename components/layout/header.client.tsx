...
export function GroupBadge({ currentGroup, allGroups, onSelect }: GroupBadgeProps) {
        ...
        >
          ...
          >
            {allGroups.map((group) => (
              <DropdownMenuItem 
                key={group.id} 
                className={cn("p-0 hover:bg-transparent space-y-1", currentGroup?.id === group.id && "bg-gray-200")}
              >
        ...
        >
          <DropdownMenuItem className="p-0 hover:bg-transparent">
            <button
              ...
              onClick={() => toast.info("We're working on it!", {
                description: "Help is coming soon!",
                duration: 10000,
                icon: <HelpCircle size={15} className="text-muted-foreground" />,
                action: {
                  label: "Got it",
